...
namespace RevToGOSTv0
{
	class GOST_21_110_2013 : IGostData
	{
		...
		*/
		...
		public GOST_21_110_2013(ElementSet elemSet)
		{
			ElemSet = elemSet;
			Position = 1;
		}
		...
		{
			Lines = new List<Line>();
			ElementSetIterator it = ElemSet.ForwardIterator();
			while (it.MoveNext())
			{
				Element elem = (Element)it.Current;
				if (ElementHasDuplicate(elem, ElemSet, it))
					continue;
				Line line = new Line();
		...
		private bool ElementHasDuplicate(Element elem, ElementSet elemSet, ElementSetIterator it)
		{
			ElementSetIterator its = elemSet.ForwardIterator();
			//while (its.MoveNext() && its != it)
			for (int i = 0; its.MoveNext() && its != it; ++i)
				if (its.Current.Equals(it.Current))
				{
					Lines[i].Amount += 1.0;
					return true;
				}
